...
- **digest 指定が通る。** `docker.io/sauroctone/nuance-mapper@sha256:3d78fd00…` を受け付けた。タグの曖昧さを構造的に排除できる
- **private リポジトリからの pull も成功する。** コンテナレジストリアクセス設定に Docker Hub の認証情報を入れれば通る
...
**digest 指定はこの意味論と特に相性が良い。** イメージが変われば参照文字列も必ず変わるため、バージョンが作られないという失敗が原理的に起きない。レジストリ側のタグ運用（可変か不変か）に依存せず、AppRunの構成文字列だけで正しさが閉じる。
...
当初、DNS解決に毎回410〜455msかかっている点を「AppRunのURLのTTLが短く、レビュアー全員が待たされるのでは」と疑ったが、**切り分けの結果これは否定された。計測環境固有のノイズである。**

| 経路 | AppRun | www.sakura.ad.jp |
| --- | --- | --- |
| ローカルリゾルバ（`10.255.255.254`、キャッシュ消去後） | 608.6ms | 960.5ms |
| Cloudflare `1.1.1.1` へ直接 | 55.8ms | 28.0ms |

- **AppRun固有ではない。** 同一環境からの `www.sakura.ad.jp` はむしろもっと遅い
- AppRunのTTLは CNAME 3600秒 / A 60秒。比較対象の `www.sakura.ad.jp` の方が A 9秒と短く、**極端に短いわけではない**
- `resolvectl query --cache=no` が106秒経っても完了せず中断。ローカルDNS経路は遅いだけでなく不安定でもある（リゾルバのアドレスからしてWSL2のスタブと思われる）

IPを `curl --resolve` で固定してDNSを完全に迂回すると:

| | dns | tls | ttfb | total |
| --- | --- | --- | --- | --- |
| 固定1 | 0.000011 | 0.0195 | 0.0705 | 0.0739 |
| 固定2 | 0.000012 | 0.0211 | 0.0706 | 0.0707 |
| 固定3 | 0.000014 | 0.0200 | 0.0710 | 0.0711 |

**ウォーム時の総時間は71〜74ms。** 先の0.3〜0.5秒はほぼ全てローカルDNSの遅延だった。計測は今後もIP固定で行う。

### コールドスタート

メトリクスでインスタンス数0を確認したうえで測定した、最初の確定サンプル（2026-08-13 18:28:01 JST）。

```
cold: http=200 dns=0.198 tls=0.218 ttfb=9.308 total=9.308 revision=overwrite-test
warm: TTFB-TLS = 50.6ms / 48.9ms / 48.3ms
```

| | 基盤側待ち時間（TTFB − TLS） |
| --- | --- |
| コールド（インスタンス0から） | **9.09秒** |
| ウォーム | 48〜51ms |

**約180倍。** 利用者視点では9.3秒待たされる。

内訳について重要な点: **この9秒にアプリの起動時間はほとんど寄与していない。** 再起動ループ中のログで Next.js は一貫して `✓ Ready in 0ms` と報告していた。したがって9秒の大半はインスタンスのスケジューリングとイメージ取得であり、**アプリ側の最適化では縮められない**性質の待ち時間である。イメージサイズの寄与だけは、極小イメージで同じ測定をすれば切り分けられる。

n=1 なので固定値としては扱わない。範囲と中央値を出すにはあと2〜4セット必要。

以前観測した13.84秒 / 13.56秒は、インスタンス数0を事前確認していないため参考値のまま。「コールドスタートは必ず13秒」とは言えず、現時点で確定しているのは9.31秒の1サンプルのみ。

**未記録の重要な値: 無通信からインスタンス数が0に落ちるまでの時間。** これがプレビュー環境の設計を分ける。

- 短い（数分）場合、CIからウォームアップリクエストを打ってもレビュアーが開く頃には冷めており無意味。打てる手は「初回は10秒ほどかかる」とPRコメントに書くことくらい
- 長い場合、デプロイ直後のウォームアップに意味が出る
- `min_scale = 1` にすれば解消するが、プレビュー4本分の常時課金が発生し、ゼロスケールで費用を抑えるという前提が崩れる
...
### 未確認・要検証
- コールドスタートの範囲と中央値（あと2〜4セット）。**無通信から0に落ちるまでの所要時間**もここで取る
- コールドスタート9秒のうちイメージ取得が占める割合（極小イメージとの比較で切り分け）
- Terraform 経由でバージョンを追加した場合もトラフィックが自動で新バージョンへ向くのか（`traffics` を明示している現構成では表面化しないが、挙動としては確認したい）
- Docker Hub の immutable tags を有効にした場合、タグの**削除**が可能かどうか。PR close 時にプレビュー用タグを消す運用と衝突しうる

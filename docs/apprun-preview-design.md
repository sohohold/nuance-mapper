...
**未判定**: 構成差分を作って新バージョンを作成した場合、同一タグの digest を引き直すのか、以前解決した digest を使い続けるのか。`/api/health` の `revision` で判定できるが、計測時に本文を取り損ねたため持ち越し。
...
1回目を分解すると TLS 完了 0.216s → TTFB 0.288s なので、**サーバー処理時間は約72ms**。今後は全計測で `time_appconnect` と `time_starttransfer` を出し、その差で比較する。

別途 13.56s を観測しているが、同様にインスタンス数0を確認していないため参考値。

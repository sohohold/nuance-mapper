...
DNS測定後、しばらくHTTPアクセスしていない状態での最初のリクエスト:

```
http=200  dns=0.071  tls=0.092  ttfb=13.844  total=13.844
```

**TLS確立後から応答開始まで約13.75秒**。直後のウォーム応答が71msなので、200倍近い差がある。過去に観測した13.56秒とも一致しており、挙動としてはコールドスタートで間違いなさそうだが、リクエスト直前にインスタンス数0を確認していないため確定値にはしない。

13秒台という値がそのままなら、**プレビュー用途では体験に直結する**。レビュアーがPRのリンクを踏んで最初に見るのがこの待ち時間になる。本測定ではインスタンス数0と、0に落ちるまでの所要時間を記録したうえで同じ内訳を取る。
...
### 未確認・要検証
- **コールドスタートの確定値**（インスタンス数0を確認したうえでの本測定、3〜5セット）と、無通信から0に落ちるまでの所要時間

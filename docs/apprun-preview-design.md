...
- **同一タグの上書き push で新バージョンが作られるか。** 上記の誤診により未検証のまま。「バージョンは構成情報が変化した時に作られる」という仕様からは、イメージ参照文字列が変わらない上書き push では作られないと読めるが、確認していない。いずれにせよ CI では一意なタグ（コミットSHA、プレビューは `pr-<N>-<sha>`）を使うので、実運用上は踏まない

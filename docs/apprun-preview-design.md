...
`:test` を上書き push して再デプロイしたところ、手元の digest（`sha256:22fb4211…`、`/api/health` が 200 を返すことを確認済み）と AppRun 上の挙動が食い違い、404 のままだった。

- **バージョンは「構成情報が変化した時」に作られる。イメージ参照文字列が同じ上書き push は構成情報の変化にあたらない**ため、新しいバージョンが作られないか、作られても最初に解決した digest のまま pull される
- **タグは必ず一意にする**（コミットSHA）。本番の `deploy-sakura.yml` が既にそうしているのは結果的に正しかった。プレビューも同様に `pr-<N>-<sha>` とする
- コントロールパネルに出たエラーは `Component is exited: ExitCode143`。**143 = 128 + SIGTERM(15)** で、アプリが自分で落ちたのではなく AppRun がヘルスチェック不通過のコンテナを終了させたことを示す。アプリ側のクラッシュと区別できる唯一の手がかりがこの終了コードだけだった
- digest 指定（`docker.io/<user>/<repo>@sha256:…`）が受け付けられるかは未確認

この切り分けに時間がかかったのは、**稼働中のインスタンスがどのビルドかを外から確認する手段が無かった**ため。対策として `/api/health` が `BUILD_REVISION`（コミットSHA、Dockerfile の build-arg 経由）を返すようにした。プレビューでは PR コメントに載せた SHA と突き合わせれば、古いイメージが動いていることに即座に気づける。

### 未確認・要検証

- Terraform 経由でバージョンを追加した場合もトラフィックが自動で新バージョンへ向くのか（`traffics` を明示している現構成では表面化しないが、挙動としては確認したい）
- イメージの digest 指定が通るか。通るならタグの曖昧さを完全に排除できる

...
`preview` ラベルを付けたPRごとに、さくらのクラウドAppRun上へ独立したプレビュー環境を払い出します。PRを閉じるかラベルを外すと削除されます。
...
枠が埋まった状態で新しいプレビューを作ろうとすると、ワークフローはエラーで停止します。他のPRのプレビューを自動で削除することはしません。ラベルを外すかPRを閉じて枠を空けてください。
...
Issues タブ → Labels → New label。名前は小文字で `preview` ちょうどにしてください。ワークフローはこの文字列と完全一致で判定するため、`Preview` や `preview-env` では起動しません。
...
PRに `preview` ラベルを付けると `nuance-mapper-pr-<PR番号>` というAppRunアプリが作られます。以降そのPRへpushするたびに更新されます。
...
| 起動条件 | `preview` ラベルによるオプトイン | アプリ数上限を人手で制御する |
